...
import type { RunnableTimerConfig } from "./validation";
...
function intervalInput(
  overrides: Partial<
    Extract<RunnableTimerConfig, { timerType: "interval" }>
  > = {},
): TimerStartInput {
  return {
    config: {
...
describe("buildTimerPlan", () => {
  ...
  it("prepares once and inserts one set rest between two sets", () => {
...
describe("timer transitions", () => {
  ...
  it("crosses multiple phases without rebasing their deadlines", () => {
    const state = startTimer(intervalInput({ cycles: 3, sets: 1 }), 5_000);
    ...
    expect(update.transitions.map((transition) => transition.type)).toEqual([
      "phase-changed",
      "phase-changed",
      "phase-changed",
      "phase-changed",
  ...
  it("keeps progress and remaining bounded if the wall clock moves back", () => {
    const state = startTimer(countdownInput(), 10_000);
    const update = tickTimer(state, 5_000);
  ...
  it("never returns to an elapsed phase when the wall clock moves back", () => {
    const state = startTimer(intervalInput({ cycles: 2 }), 0);
    ...
    const clockRollback = tickTimer(work, 5_000);

    expect(clockRollback.state).toMatchObject({
      status: "running",
      phase: { kind: "work", cycle: 1 },
      progress: 0,
      remainingSeconds: 20,
    });
    expect(clockRollback.transitions).toEqual([]);
  });

  it.each<TimerPhaseKind>(["prepare", "work", "cycle-rest", "set-rest"])(
    "cancels from %s",
    (phaseKind) => {
      const state = startTimer(intervalInput({ cycles: 2, sets: 2 }), 0);
      const phase = state.plan.phases.find(
        (candidate) => candidate.kind === phaseKind,
      );

      expect(phase).toBeDefined();
      const phaseIndex = state.plan.phases.indexOf(phase!);
      const phaseStart = state.plan.phases
        .slice(0, phaseIndex)
        .reduce(
          (total, candidate) => total + candidate.durationSeconds * 1_000,
          0,
        );
      const phaseState = tickTimer(state, phaseStart).state;

      expect(phaseState).toMatchObject({
        status: "running",
        phase: { kind: phaseKind },
      });
      expect(cancelTimer(phaseState)).toBe(idleTimerState);
    },
  );
});

describe("createTimerEngine", () => {
  it("injects its clock without retaining mutable timer state", () => {
    let now = 10_000;
